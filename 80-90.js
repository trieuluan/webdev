requirejs([
    ...
    class PlayGame {
        ...
        static chanceGo = {min: 5, max: 49.5};
        static listGameGo = [];
        ...
        static golandau = 10;
        static goFlag = false;
        static defaultBasebetGo = 0;
        static target = 50;
        static defaultChange = ()=>{
            return PlayGame.randomNumberFromRange(80, 90);
        };
        ...
        constructor(){
            ...
            $('.game__left__content').on('click', '#btn-start-bot', function () {
                ...
                if(PlayGame.defaultBasebet === 0){
                    ...
                    PlayGame.defaultBasebetGo = PlayGame.defaultBasebet / 10;
                }
        ...
        createTool = () => {
            ...
            var headNameBot = $('<h1/>', {text: 'Super Bot Dice', style: 'color: white;'}).append('<br/>').append($('<small/>', {text: 'Chance 80 - 90 v2.2', style: 'color: white;'}));
        ...
        static getGoChange(){
            ...
            PlayGame.listGameGo.forEach(function (item) {
                ['hi', 'low'].forEach(function (condition) {
                    if (item[condition].tilethang >= phantram && item[condition].tilethang > 5 && item[condition].tilethang < 10){
        ...
        static updateChanceLose = () => {
            ...
            if(PlayGame.storeBalanceLose / PlayGame.bigBalance * 100 >= 1){
                PlayGame.balanceLose = PlayGame.storeBalanceLose;
                PlayGame.storeBalanceLose = 0;
                PlayGame.basebet = PlayGame.defaultBasebetGo;
            }
            ...
            if(PlayGame.balanceLose > 0){
                ...
                if(go.tilethang > 5 && go.tilethang < 10){
                    if(PlayGame.balanceLose / PlayGame.bigBalance * 100 < 10){
                        PlayGame.basebet = parseFloat(PlayGame.balanceLose / (getPayout - 1));
                    }else{
                        PlayGame.basebet = parseFloat(PlayGame.balanceLose / 5 / (getPayout - 1));
                    }
                    PlayGame.basebet = PlayGame.basebet + PlayGame.basebet * go.onlose / 100;
                    PlayGame.chance = go.change;
                    PlayGame.defaultHi = go.defaultHi;
                }else{
                    // let lostTang = PlayGame.basebet / (PlayGame.basebet * parseFloat(PlayGame.getPayout(PlayGame.chance)) - PlayGame.basebet) * 120;
                    PlayGame.basebet = PlayGame.defaultBasebetGo;
                }
            }else{
        ...
        static updateChanceWin = ()=>{
            ...
            if(PlayGame.goFlag){
                PlayGame.basebet = PlayGame.defaultBasebetGo;
                PlayGame.chance = PlayGame.randomNumberFromRange(5, 90);
            }else{
                PlayGame.basebet = PlayGame.defaultBasebet;
                    ...
                    success: function (text){
                        ...
                        if (val.return.success === 'true'){
                            ...
                            //         success: function (mygame){
                                        ...
                                        }else if (PlayGame.basebet < 0.00015000 && devise === 'xrp'){
                                            PlayGame.basebet = 0.00015000;
                                        }else if (PlayGame.basebet < 0.00015000 && devise === 'qtum'){
                                            PlayGame.basebet = 0.00001000;
                                        }
