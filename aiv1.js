requirejs([
    ...
    class PlayGame {
        ...
        loadData(): Promise<any>{
            this.from = this.callPrompt('Nhập min chance', 1);
            this.to = this.callPrompt('Nhập max chance', 90);
            ...
            if(run){
                ...
                $(".loader-page").show();
                return $.ajax({
                    url: API_URL+'load-data-risk/'+this.seedsHash,
        ...
        updateTilethang(roll_number): void{
            this.dataRoll = _.forEach(this.dataRoll, (val) => {
                if(val.defaultHi){
                    if(roll_number < val.game){
                        val.lose += 1;
                    }else{
                        val.lose = 0;
                    }
                ...
                val.tilethang = val.lose / val.risk * 100;
            });
        }
        loadDatabet():void {
            let filter = _.filter(this.dataRoll, (o) => {
                return o.tilethang <= 10 || (o.tilethang >= 20 && o.tilethang <= 30) || (o.tilethang >= 50 && o.tilethang <= 60) || o.tilethang >= 80;
            });
            let sort = _.orderBy(filter, ['tilethang'], ['desc']);
            let bet = _.head(sort);
            // if(bet.chance > 66){
            //     bet.chance = 98-bet.chance;
            //     bet.defaultHi = !bet.defaultHi;
            //     bet.game = bet.defaultHi ? parseFloat((100-bet.chance-0.01)).toFixed(2) : parseFloat(bet.chance).toFixed(2);
            //     bet.condition = bet.defaultHi ? '>' : '<';
            //     let payout = this.getPayout(bet.chance);
            //     bet.onlose = 1 / (1 * payout - 1) * 120;
            // }
            return bet;
        }
        doScript(amount_return): void{
            ...
            if (amount_return >= 0){
                ...
                this.lose = 0;
                if(this.balanceLose > 0 && this.flagGo){
                ...
                if(this.databet.tilethang >= 80){
                    this.flagGo = true;
                    this.basebet = this.balanceLose / 3 / (payout - 1);
                }else if(this.databet.tilethang >= 50 && this.databet.tilethang <= 60){
                    this.flagGo = true;
                    this.basebet = this.balanceLose / 6 / (payout - 1);
                }else if(this.databet.tilethang >= 20 && this.databet.tilethang <= 30){
                    this.flagGo = true;
                    this.basebet = this.balanceLose / (payout - 1);
                }else{
                    this.flagGo = false;
            ...
            } else if(this.basebet < 0.00000020 && (this.coin === 'xmr' || this.coin === 'dash' || this.coin === 'zec')){
