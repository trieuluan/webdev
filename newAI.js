requirejs([
    ...
    class PlayGame {
        ...
        createTool (): void {
            ...
            var div100RunBalanceLose = (div100.clone()).append('<h5>Run Balance Lose</h5>');
            div100RunBalanceLose.append($('<div/>', {id: 'run-balance-lose', class: 'stats__div__infos', text: 0}));
            ...
            flex5.attr('id', 'flexrow5').append(div100RunBalanceLose);
        ...
        loadDatabet():void {
            ...
            if (!_.isEmpty(this.dataRoll)){
                ...
                let sortHi = _.orderBy(rollHi, ['slxh'], ['asc']);
                let sortLow = _.orderBy(rollLow, ['slxh'], ['asc']);
                let sortNormal = _.orderBy(rollNormal, ['slxh'], ['asc']);
                let roll = [];
        ...
        doScript(amount_return): void{
            ...
            if (amount_return >= 0){
                ...
                this.win = 0;
                if(this.flagGo){
                    this.runBalanceLose += this.basebet;
                }else{
            ...
            if(this.runBalanceLose === 0 && this.balanceLose / this.currentBalance * 100 >= 0.1){
                this.runBalanceLose += (Math.floor(Math.random() * 100) + 10) * this.balanceLose / 100;
                this.balanceLose -= this.runBalanceLose;
            }
            if(this.runBalanceLose / this.currentBalance * 100 >= 10 && !this.notCare){
                this.notCare = true;
                this.balanceLose += 10 * this.runBalanceLose / 100;
                this.runBalanceLose = 90 * this.runBalanceLose / 100;
            }
            if(this.runBalanceLose > 0){
                this.flagGo = true;
                this.basebet = this.runBalanceLose / (payout);
                this.basebet = this.basebet + this.basebet * this.databet.onlose / 100;
            }
            ...
            if(this.runBalanceLose <= 0){
                this.runBalanceLose = 0;
                this.onRunRollNumber = 1;
            }
            ...
            if (val.return.success === 'true'){
                ...
                this.infoSeeds(id).then(resText => {
                    ...
                    this.insertBet(dataPush).then(gameData => {
                        ...
                        $('#run-balance-lose').html(parseFloat(this.runBalanceLose).toFixed(8));
                        if(amount_return >= 0){
